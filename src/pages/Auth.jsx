...
const C = {
  ...
  red:'#f05252', blue:'#5b8def', purple:'#9f7aea',
  text:'#e6eaf4', textSub:'#8496b5', textMuted:'#445570',
}
...
export default function Auth() {
  ...
  const handleForgot = async (e) => {
    ...
    else setMsg('📧 Link đặt lại mật khẩu đã gửi vào email của bạn!')
    setLoading(false)
  }
  ...
  const inp = {
    ...
    borderRadius:14, padding:'13px 16px', color:C.text, fontSize:15,
    outline:'none', boxSizing:'border-box', fontFamily:'inherit', transition:'border .15s',
  }
  ...
  const btn = (bg) => ({
    width:'100%', background:bg, border:'none', borderRadius:14, padding:'15px 0',
    color: bg === C.accent ? '#07090f' : C.text, fontWeight:800, fontSize:15,
    cursor:'pointer', fontFamily:'inherit', boxShadow:`0 4px 14px ${bg}44`,
  ...
  if (screen === 'verify') return (
    <div style={{minHeight:'100vh', background:C.bg, display:'flex', alignItems:'center', justifyContent:'center', padding:20}}>
      <div style={{width:'100%', maxWidth:400, textAlign:'center'}}>
        ...
        <div style={{background:C.card, border:`1px solid ${C.border}`, borderRadius:18, padding:'16px 20px', marginBottom:20, fontSize:13, color:C.textSub, textAlign:'left', lineHeight:1.7}}>
          1. Mở email và tìm thư từ <strong style={{color:C.text}}>Finance App</strong><br/>
          2. Nhấn vào link <strong style={{color:C.green}}>Xác nhận email</strong><br/>
  ...
  return (
    <div style={{minHeight:'100vh', background:C.bg, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px', paddingTop:40}}>
      <div style={{width:'100%', maxWidth:420}}>
        ...
        <div style={{background:C.card, border:`1.5px solid ${C.borderLight}`, borderRadius:26, padding:'24px 22px 28px', boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}>
            ...
            {[['login','🔐 Đăng nhập'],['register','✨ Đăng ký']].map(([m,label])=>(
              <button key={m} onClick={()=>{setScreen(m);reset()}}
                style={{flex:1, padding:'11px 0', borderRadius:13, border:'none', cursor:'pointer',
                  ...
                  background: screen===m ? C.accent : 'transparent',
                  color: screen===m ? '#07090f' : C.textMuted,
          ...
          {error && <div style={{background:`${C.red}18`, border:`1px solid ${C.red}44`, borderRadius:12, padding:'11px 14px', marginBottom:16, fontSize:13, color:C.red, display:'flex', gap:8, alignItems:'flex-start'}}>
          ...
          {screen === 'login' && (
            <form onSubmit={handleLogin}>
              <div style={{display:'flex', flexDirection:'column', gap:13}}>
                ...
                <button type="submit" disabled={loading} style={btn(C.accent)}>
          ...
          {screen === 'register' && (
            <form onSubmit={handleRegister}>
              <div style={{display:'flex', flexDirection:'column', gap:13}}>
                ...
                <div><span style={lbl}>Mật khẩu</span>
                  ...
                  {password && (
                    <div style={{marginTop:6, display:'flex', gap:4}}>
                      ...
                      <span style={{fontSize:10, color:password.length>=12?C.green:password.length>=8?C.accent:C.red, fontWeight:700, marginLeft:4}}>
                ...
                <button type="submit" disabled={loading} style={btn(C.green)}>
          ...
          {/* Google */}
          <button onClick={handleGoogle} style={{width:'100%', background:C.surface, border:`1.5px solid ${C.border}`, borderRadius:14, padding:'13px 0', color:C.text, fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', justifyContent:'center', gap:10, transition:'border .2s'}}>
          ...
          {screen === 'login' && (
            <div style={{textAlign:'center', marginTop:16, fontSize:13, color:C.textMuted}}>
              Chưa có tài khoản?{' '}
              <button onClick={()=>{setScreen('register');reset()}} style={link}>Đăng ký ngay</button>
            </div>
          )}
          {screen === 'register' && (
            <div style={{textAlign:'center', marginTop:16, fontSize:13, color:C.textMuted}}>
              Đã có tài khoản?{' '}
              <button onClick={()=>{setScreen('login');reset()}} style={link}>Đăng nhập</button>
            </div>
          )}
        </div>

        <div style={{textAlign:'center', marginTop:20, fontSize:11, color:C.textMuted, lineHeight:1.6}}>
          Bảo mật dữ liệu bởi Supabase • Mã hóa end-to-end<br/>
          Mỗi người dùng chỉ thấy dữ liệu của riêng mình
        </div>
      </div>
    </div>
  )
